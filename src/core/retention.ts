...
function pickNudges(params: {
  ...
  if (params.playedToday && params.fatigue >= 55) {
    out.push({
      ...
      body: 'Ещё один заход сегодня скорее накопит усталость, чем усилит навык. Отдых — часть честного прогресса.',
      priority: 90
    });
  }
  ...
  if (params.neglect >= 50 && params.neglectedDomain) {
    ...
    out.push({
      id: 'rebalance',
      kind: 'rebalance',
      title: 'Тренировка отстающего',
      body: `Зона «${name}» давно не тренировалась. Сегодня она в фокусе, чтобы вернуть баланс способностей.`,
      priority: 60
    });
  }
