...
export function playCue(id: CueId, ctx: CueContext = {}, prefs?: SoundPrefs): ScheduledCue {
  const resolved = prefs ?? prefsFromStorage();
  const scheduled = scheduleCue(id, resolved, ctx);
  renderScheduled(scheduled);
  if (!scheduled.skipHaptic) vibrateForCue(id, resolved, ctx);
  return scheduled;
}
