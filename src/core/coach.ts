...
export function getDailySpark(params: {
  ...
  if (!calibrated) {
    return {
      ...
      body: 'Пройдите калибровку (около 90 секунд), чтобы Fokus узнал вашу стартовую скорость и точность. Это защитит от слишком лёгких или фрустрирующих блоков.',
      tone: 'start'
    };
  }
  ...
  if (playedToday) {
    ...
    return {
      title: 'План выполнен',
      body: 'Когнитивные навыки растут от регулярности, не от марафонов. Завтра Fokus соберёт новую сессию с учётом сегодняшних данных.',
      tone: 'habit'
    };
  }
  ...
  if (skippedYesterday && streak > 0) {
    return {
      ...
      body: 'Один пропуск Fokus прощает без штрафов. Короткий блок сегодня закрепит привычку надёжнее, чем долгий марафон на выходных.',
      tone: 'recovery'
    };
  }
  ...
  if (streak === 0 && daySummaries.length > 0) {
    return {
      ...
      body: 'Не пытайтесь наверстать упущенное за один раз. Знакомый короткий блок поможет плавно вернуться в рабочий ритм.',
      tone: 'recovery'
    };
  }
  ...
  if (daySummaries.length === 0) {
    return {
      title: 'Первый ритуал',
      body: 'Внимание тренируется как мышца. Пройдите первую сессию — Fokus начнёт собирать статистику и подстраивать нагрузку под ваш ритм.',
      tone: 'start'
    };
  }
