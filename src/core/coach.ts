...
export function getDailySpark(params: {
  ...
  if (playedToday) {
    ...
    return {
      title: 'План выполнен',
      body: 'Навыки растут от регулярности. Завтра Fokus соберёт новую сессию с учётом сегодняшних данных.',
      tone: 'habit'
    };
  }
  ...
  if (skippedYesterday && streak > 0) {
    return {
      ...
      body: 'Один пропуск не ломает ритм. Короткий блок сегодня закрепит привычку надёжнее.',
      tone: 'recovery'
    };
  }
  ...
  if (streak === 0 && daySummaries.length > 0) {
    return {
      ...
      body: 'Не пытайтесь наверстать всё за один раз. Знакомый короткий блок поможет плавно вернуться в ритм.',
      tone: 'recovery'
    };
  }
  ...
  if (chrono.bucket && chrono.sample >= 3) {
    ...
    if (nowBucket === chrono.bucket) {
      return {
        ...
        body: `По прошлым сессиям вы точнее ${chrono.label}. Сегодня хорошее время для сложного блока.`,
        tone: 'time'
      };
    }
  }
  ...
  if (streak >= 7) {
    return {
      ...
      body: 'Регулярность важнее интенсивности. Короткая сессия каждый день полезнее редких рывков.',
      tone: 'habit'
    };
  }
