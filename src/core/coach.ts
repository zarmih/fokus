...
export function getWeeklyDomainTips(domainId: string): string[] {
  switch (domainId) {
    case 'memory':
      return [
        'Попробуйте удержать список покупок в уме, не доставая телефон до кассы.',
        'Вспоминайте детали вчерашнего дня в обратном порядке перед сном.'
      ];
    case 'attention':
      return [
        'Уберите телефон из поля зрения на время работы — даже выключенный, он отнимает часть фокуса.',
        'Замечайте момент, когда мысль уходит в сторону при чтении, и просто мягко возвращайте её назад.'
      ];
    case 'logic':
      return [
        'Прежде чем гуглить ответ, дайте себе 30 секунд, чтобы вывести его логически.',
        'Объясните сложную концепцию из своей работы человеку не из вашей сферы.'
      ];
    case 'speed':
      return [
        'Легкая физическая разминка перед умственной задачей временно ускоряет обработку информации.',
        'Установите мягкий таймер на рутинную задачу и попробуйте уложиться чуть быстрее обычного.'
      ];
    case 'spatial':
      return [
        'Попробуйте пройти привычный маршрут, мысленно рисуя его карту сверху.',
        'Складывая вещи или упаковывая сумку, заранее представляйте, как они оптимально поместятся.'
      ];
    case 'math':
      return [
        'Считайте примерную сумму покупок в корзине до подхода к кассе.',
        'Оценивайте оставшееся время в пути без навигатора, исходя из пройденного расстояния.'
      ];
    default:
      return [
        'Регулярность важнее интенсивности. Короткие тренировки работают лучше долгих марафонов.',
        'Качественный сон — лучшее, что вы можете сделать для закрепления нейронных связей.'
      ];
  }
}
