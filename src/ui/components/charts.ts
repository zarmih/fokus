...
export function renderRadarChart(slices: DomainSlice[], opts?: { size?: number; max?: number }): string {
  ...
  const axes = slices.map((_, i) => {
    ...
    return `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="var(--line)" stroke-width="1" opacity="0.5"/>`;
  }).join('');
  ...
  return `
    <svg class="radar-svg" viewBox="0 0 ${view} ${view}" role="img" aria-label="Когнитивный профиль: ${slices.map((s) => `${domainLabel(s.id)} ${s.ready ? Math.round(s.value) : 'нет данных'}`).join(', ')}">
      ${rings}
      ${axes}
      <polygon points="${valuePts}" fill="var(--accent-glow-light)" stroke="var(--accent)" stroke-width="1.5"/>
      ${dots}
      ${labels}
    </svg>
  `;
