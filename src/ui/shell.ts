  ...
  const navHtml = params.hideNav ? '' : `
    <div class="tab-bar">
      ...
      <div class="tab-item ${params.active === 'trainers' ? 'active' : ''}" id="tab-trainers">
        ...
        <span>${t('trainers.title')}</span>
      </div>
      <div class="tab-item ${params.active === 'progress' ? 'active' : ''}" id="tab-progress">
        <svg viewBox="0 0 24 24"><path d="M16 6l2.29 2.29-4.88 4.88-4-4L2 16.59 3.41 18l6-6 4 4 6.3-6.29L22 12V6z"/></svg>
        <span>${t('analytics.title')}</span>
      </div>
      <div class="tab-item ${params.active === 'duel' ? 'active' : ''}" id="tab-duel">
        <svg viewBox="0 0 24 24"><path d="M12 2L4 5v6c0 5.55 3.84 10.74 8 12 4.16-1.26 8-6.45 8-12V5l-8-3zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12.99H5V6.3l7-2.62v9.31z"/></svg>
        <span>${t('duel.title')}</span>
      </div>
