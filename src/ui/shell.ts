...
export function renderShell(container: HTMLElement, params: {active: 'today' | 'program' | 'trainers' | 'progress' | 'duel' | 'settings', hideNav?: boolean}): HTMLElement {
  ...
  const tab = (id: typeof params.active, label: string, path: string) => {
    // If active is duel or program, highlight trainers (Catalog)
    const isActive = params.active === id || (id === 'trainers' && (params.active === 'duel' || params.active === 'program'));
    return `
      <button type="button" class="tab-item ${isActive ? 'active' : ''}" id="tab-${id}" ${isActive ? 'aria-current="page"' : ''}>
