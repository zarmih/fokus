...
export function renderTrainers(container: HTMLElement) {
  ...
  content.innerHTML = `
    <style>
      ...
      .catalog-section { margin-bottom: 32px; animation: fadeIn var(--motion-base) var(--ease-enter); }
      ...
      .catalog-section-grid.dense { grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 10px; }
      ...
      .domain-filters { margin-bottom: 24px; display: flex; gap: 8px; padding: 0 20px; overflow-x: auto; scrollbar-width: none; }
      .domain-filters::-webkit-scrollbar { display: none; }
      .filter-chip { flex: 0 0 auto; border: 1px solid var(--line); background: var(--surface); color: var(--muted); border-radius: 999px; padding: 8px 12px; font-size: 13px; font-weight: 700; cursor: pointer; transition: all 0.2s; }
      .filter-chip.active { background: var(--accent); color: var(--btn-text); border-color: transparent; }
    </style>
    
    <div class="today-head" style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: flex-start; padding: 0 20px;">
      <div>
        <h2 style="margin: 0 0 8px;">Каталог тренажёров</h2>
        <p class="today-date" style="margin: 0;">${catalog.length} упражнений. Практика без влияния на Fokus Index.</p>
      </div>
      <button id="density-toggle" style="background: var(--surface-2); border: 1px solid var(--line); color: var(--text); padding: 8px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; margin-top: 4px; transition: transform 0.2s, background 0.2s;" aria-label="Переключить вид">
      </button>
    </div>
  ...
  const densityBtn = content.querySelector('#density-toggle') as HTMLButtonElement;
  ...
  const getExerciseCardHtml = (ex: any) => {
    ...
    if (intel.state === 'CALIBRATING') {
      intelHtml = `
        <div style="margin-top: ${isDense ? 'auto' : '12px'};">
          ...
          ${isDense ? '' : `<div style="font-size: 11px; color: var(--muted); margin-top: 4px;">Сложность: ${intel.difficulty}</div>`}
        </div>`;
    } else {
    ...
    return `
      <button type="button" class="trainer-card press-physics dom-${ex.manifest.domain}" data-id="${ex.manifest.id}" aria-label="${ex.manifest.name}, ${domainLabel(ex.manifest.domain as any)}, уровень ${lvl}" style="${isDense ? 'min-height: 100px; padding: 12px;' : ''}">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: ${isDense ? '8px' : '0'};">
          <div class="trainer-domain" style="display: ${isDense ? 'none' : 'block'};">${domainLabel(ex.manifest.domain as any)}</div>
          ${isDense ? `<div class="trainer-name" style="margin-bottom:0; font-size: 14px; text-align: left; padding-right: 8px;">${ex.manifest.name}</div>` : ''}
          <img src="${import.meta.env.BASE_URL}art/icon-${ex.manifest.id}.svg" width="${isDense ? '28' : '32'}" height="${isDense ? '28' : '32'}" alt="" decoding="async" loading="lazy" style="border-radius: ${isDense ? '6px' : '8px'}; flex-shrink: 0;">
        </div>
        ${isDense ? '' : `
        <div class="trainer-name">${ex.manifest.name}</div>
        <div class="trainer-instruction" style="font-size: 12px; color: var(--muted); margin-bottom: 8px; line-height: 1.4;">${ex.manifest.instruction}</div>
  ...
  const renderCatalog = () => {
    ...
    if (!searchQuery && currentDomain === 'all') {
        ...
        return `
          <div class="catalog-section">
            <h3 class="catalog-section-title">
              <div style="width: 10px; height: 10px; border-radius: 3px; background: var(--dom-${dom}); box-shadow: 0 0 8px var(--dom-${dom}); margin-right: 4px;"></div>
              ${domName} <span style="opacity:0.3; font-size:14px; font-weight:normal; margin-left: 4px;">${exs.length}</span>
            </h3>
            <div class="catalog-section-grid ${isDense ? 'dense' : ''}">
      ...
      contentContainer.innerHTML = `
        <div class="catalog-section-grid ${isDense ? 'dense' : ''}" style="padding-bottom: 32px;">
