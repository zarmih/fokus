...
import { scoreSessionQuality, qualityBand } from '../../core/sessionQuality';
...
export function renderResult(container: HTMLElement, params: { session: Session; calibration?: boolean; recalibration?: boolean; unlocked?: string[] }) {
  ...
  const feedbackHtml = (!isCalibration && session.items.length > 0) ? `
    <div class="surface quality-feedback">
      <h3>Качество сессии</h3>
      <div style="font-size: 24px; font-weight: 600; margin-bottom: 4px;">${quality.score} / 100</div>
      <p class="muted">${qualityBand(quality.score)}</p>
    </div>
  ` : '';
  ...
  content.innerHTML = `
    ...
    <div class="result-actions">
      ...
      ${nextEx && !isCalibration ? `<button id="btn-next" class="btn-primary">Далее: ${nextEx.name}</button>` : ''}
      <button id="btn-done" class="${nextEx && !isCalibration ? 'btn-secondary' : 'btn-primary'}">Готово</button>
    </div>
  `;
